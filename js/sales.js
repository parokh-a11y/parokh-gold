...
(function () {
  ...
  if ($("pay-txid-btn")) {
    $("pay-txid-btn").addEventListener("click", async function () {
      ...
      var server = ($("pay-server") && $("pay-server").value || "").trim();
      ...
      if (!server || server.length < 2) {
        msg.textContent = isFa() ? "نام سرور MT5 الزامی است (Login → Server)." : "MT5 server name is required (Login → Server).";
        return;
      }
      if (txid.length < 20) {
      ...
      try {
        var d = await postJson("/api/payment/txid", {
        ...
        var pv = d && d.paymentVerify;
        if (pv && pv.ok) {
          msg.textContent = isFa()
            ? "پرداخت روی زنجیره تأیید شد. درخواست لایسنس ثبت شد؛ ادامه از ایمیل."
            : "On-chain payment verified. License request queued; we continue by email.";
        } else if (pv && pv.code === "pending") {
          msg.textContent = isFa()
            ? "تراکنش در حال تأیید شبکه است. کمی بعد دوباره TXID را ثبت کنید یا صبر کنید."
            : "Transaction still pending on-chain. Wait or resubmit TXID shortly.";
        } else {
          msg.textContent = isFa()
            ? "TXID و اطلاعات حساب ثبت شد (بررسی نهایی دستی/خودکار). ادامه از ایمیل."
            : "TXID and account saved (final review). We continue by email.";
        }
      } catch (err) {
