  ...
  (async function () {
    ...
    try {
      if (window.parokhAuth && parokhAuth.session) {
        var _s = await parokhAuth.session();
        if (_s && _s.user && _s.user.email) email = _s.user.email;
      }
    } catch (e) {}
    try {
      if (window.parokhAuth && parokhAuth.session) {
  ...
  function showPayment(order) {
    ...
    if (addr) {
      if (order.paymentAddress) {
        addr.textContent =  order.paymentAddress || (isFa()
          ? "آدرس رسمی پس از تنظیم ولت نمایش داده می‌شود (آدرس ثابت)."
          : "Official fixed address appears when wallet is configured.");
        if ($("copy-address")) $("copy-address").hidden = false;
      } else {
        addr.textContent = isFa()
          ? "آدرس یکتای این سفارش پس از اتصال درگاه نمایش داده می‌شود"
          : "Official fixed address is used when configured. Manual review after TXID.";
  ...
  if (form) {
    form.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        try {
          if (window.parokhAuth && parokhAuth.session) {
            var sess = await parokhAuth.session();
        ...
        if (!tok) {
          if (msg) {
            ...
            msg.textContent = isFa() ? "برای خرید کامل ابتدا وارد حساب شوید." : "Please log in before placing a full order.";
          }
          return;
        }
    ...
    txBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        try {
          if (window.parokhAuth && parokhAuth.session) {
            var s = await parokhAuth.session();
            tok = s && s.access_token;
          }
        } catch (e) {}
        await fetch("/api/orders/" + encodeURIComponent(currentOrder.id) + "/payment", {
          method: "POST",
