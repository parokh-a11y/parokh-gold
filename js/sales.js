/* PAROKH GOLD v2.0.3 — shared demo/buy form */
(function () {
    ...
    $("pay-txid-btn").addEventListener("click", async function () {
      ...
      try {
        ...
        msg.textContent = isFa()
          ? "TXID ثبت شد. پس از بررسی پرداخت، از همان ایمیل با شما ادامه می‌دهیم."
          : "TXID submitted. We will continue by the same email after payment review.";
      } catch (err) {
  ...
  async function submitKind(kind) {
    ...
    try {
      ...
      if (kind === "demo") {
        msg.style.color = "#4ade80";
        msg.textContent = isFa()
          ? ("درخواست دمو ثبت شد. کد: " + (order.id || "") + " — پیگیری از ایمیل شما انجام می‌شود. تیم ما از همین مسیر با شما در تماس خواهد بود.")
