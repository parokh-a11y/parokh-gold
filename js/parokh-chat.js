...
(function () {
  ...
  function resetState() {
    return {
      ...
      name: "", email: "", broker: "", account: "", server: "",
  ...
  function saveState() {
    try {
      sessionStorage.setItem(STORE_KEY, JSON.stringify({
        ...
        name: state.name, email: state.email, broker: state.broker, account: state.account, server: state.server,
  ...
  function ensureUi() {
    ...
    css.textContent = [
      ...
      "#pg-chat-panel{pointer-events:auto;position:fixed;top:auto;bottom:max(90px,env(safe-area-inset-bottom));",
      ...
      "#pg-chat-panel.open{display:flex}","#pg-chat-panel.pg-side-right{inset-inline-end:16px;inset-inline-start:auto;left:auto;right:16px;transform:none}","#pg-chat-panel.pg-side-left{inset-inline-start:16px;inset-inline-end:auto;left:16px;right:auto;transform:none}",
  ...
  function applyThemeClass() {
    ...
    var rtl = isFa() || document.documentElement.getAttribute("dir") === "rtl";
    // FA/RTL → panel on left; EN → on right (near bubble)
    panel.classList.toggle("pg-side-left", rtl);
    panel.classList.toggle("pg-side-right", !rtl);
  }
  function fitViewport() {
    var panel = document.getElementById("pg-chat-panel");
    if (!panel || !panel.classList.contains("open") || !window.visualViewport) return;
    var vv = window.visualViewport;
    var h = Math.min(380, Math.max(240, vv.height - 100));
  ...
  function askServer() {
    state.step = "server";
    setStatus(t("سرور MT5", "MT5 server"));
    bot(t("نام دقیق سرور را از MT5 بفرستید (Login → Server).", "Send the exact server name from MT5 (Login → Server)."));
    actions([{ label: t("منو", "Menu"), onClick: home }]);
    showInput(t("نام سرور", "Server name"));
    saveState();
  }
  function askTxid() {
    state.step = "txid";
  ...
  function onSend() {
    ...
    if (state.step === "server") {
      if (v.length < 2) {
        bot(t("نام سرور کوتاه است.", "Server name is too short."));
        showInput(t("نام سرور MT5", "MT5 server"));
        return;
      }
      state.server = v;
      askTxid();
