...
async function handleOrders(request, env) {
  ...
  if (request.method === "POST") {
    ...
    // Option B: guest checkout allowed (email form). Auth optional.
    ...
    // MT5 broker/account collected later offline — not required at request time (Option B)

    order.source = String(body.source || "site-form").slice(0, 40);
    order.replyChannel = String(body.replyChannel || "email").slice(0, 40);
    order.channelNote = "Reply via " + order.replyChannel + " · source=" + order.source;
    ...
    // Always notify admin Telegram
    try {
      const tgText = [
        "PAROKH GOLD · " + (order.kind === "demo" ? "DEMO" : "PURCHASE"),
        "ID: " + order.id,
        "Plan: " + order.plan + " · $" + order.price,
        "Name: " + order.name,
        "Email: " + order.email,
        "Source: " + order.source,
        "Reply via: " + order.replyChannel,
        "Status: " + order.status
      ].join("\n");
      await notifyTelegram(env, tgText);
    } catch (_) {}
