...
async function handlePaymentTxid(request, env) {
  ...
  try {
    await notifyTelegram(env, [
      ...
      "TXID: " + txid,
      "Source: " + (order.source || "site"),
      "Action: verify payment then build dedicated EX5"
    ].join("\n"), order.id + ":txid");
  } catch (_) {}

  return json({ ok: true, order: { id: order.id, paymentStatus: order.paymentStatus, txid: order.txid, broker: order.broker, account: order.account } }, 200, request);
}
