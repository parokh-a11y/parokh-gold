...
async function handleOrders(request, env) {
  ...
  if (request.method === "POST") {
    ...
    try {
      const lines = [
        "PAROKH GOLD · " + (order.kind === "demo" ? "DEMO REQUEST" : "PURCHASE"),
        "ID: " + order.id,
        "Plan: " + order.plan + (order.kind === "full" ? (" · $" + order.price) : ""),
        "Name: " + order.name,
        "Email: " + order.email,
        ...
        "Status: " + order.status,
        order.kind === "demo"
          ? "Action: prepare demo / reply to customer email"
          : "Action: wait for TXID then verify payment"
      ];
...
async function handlePaymentTxid(request, env) {
  ...
  return json({ ok: true, order: { id: order.id, paymentStatus: order.paymentStatus, txid: order.txid } }, 200, request);
}
