/**
 * PAROKH GOLD EA — Worker v2.0.5 — website → license_requests
...
async function handlePaymentTxid(request, env) {
  ...
  // support optional server field
  const server = String(body.server || "").trim().slice(0, 120);
  if (server) order.server = server;
  orders[idx] = order;
  await kvPutJson(env, "orders", orders);
  ...
  try {
    ...
    if (license.ok && license.row) {
      await notifyLicenseAdmins(env, license.row, {
        ...
        broker: order.broker,
        server: order.server || order.broker,
  ...
  try {
    await notifyTelegram(env, [
      ...
      "Action: verify payment then activate EX5"
    ].join("\n"), order.id + ":txid");
  } catch (_) {}

  return json({
    ok: true,
    order: { id: order.id, paymentStatus: order.paymentStatus, txid: order.txid, broker: order.broker, account: order.account },
...
async function createLicenseRequestFromSite(env, order) {
  ...
  const body = {
    ...
    server_name: order.server || order.broker || null,
    plan: plan,
    customer_email: order.email || null,
    customer_tg_id: null,
    order_ref: String(order.id || order.txid || ""),
    payment_status: order.paymentStatus === "TXID_SUBMITTED" ? "txid_submitted" : (order.paymentStatus || "pending"),
