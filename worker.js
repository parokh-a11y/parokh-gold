/**
 * PAROKH GOLD EA — Worker v2.0.10 — website → license_requests
...
function adminEmailKeyboard(order) {
  ...
  const isDemo = order.kind === "demo";
  const rows = [];
  // Merged Received+Review
  rows.push([{ text: "✅ دریافت شد", callback_data: ("em:recv:" + id).slice(0, 64) }]);
  if (isDemo) {
    rows.push([{ text: "📎 ارسال دمو", callback_data: ("em:demo:" + id).slice(0, 64) }]);
  } else {
    rows.push([
      { text: "❌ TXID نامعتبر", callback_data: ("em:badtx:" + id).slice(0, 64) },
      { text: "📦 تحویل خرید", callback_data: ("em:deliver:" + id).slice(0, 64) },
    ]);
  }
...
function formatOrderAdminHtml(order, extraLines) {
  ...
  const lines = [
    ...
    "<b>Kind:</b> " + escHtml(order.kind || "") + " · <b>Plan:</b> " + escHtml(order.plan || "") +
      (order.price != null && order.kind !== "demo" ? (" · $" + escHtml(order.price) + " USDT") : ""),
  ...
  lines.push("");
  if (order.kind === "demo") {
    lines.push("<i>دمو: دریافت شد → ارسال دمو (پیوست زیپ)</i>");
  } else {
    lines.push("<i>خرید: دریافت شد · TXID نامعتبر · تحویل خرید</i>");
  }
  return lines.join("\n");
}
...
async function notifyTelegram(env, text, dedupeKey, options) {
  ...
  try {
    ...
    if (options.parse_mode) base.parse_mode = options.parse_mode;
    if (options.reply_markup) base.reply_markup = options.reply_markup;

    let res = await sendOnce(base);
    if (!res.ok && options.reply_markup) {
      const plain = {
        chat_id: chat,
        text: String(text).slice(0, 3900),
        disable_web_page_preview: true,
      };
      if (options.parse_mode) plain.parse_mode = options.parse_mode;
      res = await sendOnce(plain);
    }
    if (!res.ok && options.parse_mode) {
      res = await sendOnce({
        chat_id: chat,
        text: String(text).replace(/<[^>]+>/g, "").slice(0, 3900),
        disable_web_page_preview: true,
      });
    }

    if (!res.ok) {
      return { ok: false, reason: "tg_http", detail: String(res.body || "").slice(0, 200) };
    }
    if (dedupeKey && env.PAROKH_KV) {
      await env.PAROKH_KV.put("tg_once:" + String(dedupeKey).slice(0, 120), "1", { expirationTtl: 120 });
    }
    return { ok: true };
  } catch (e) {
    return { ok: false, reason: String(e && e.message ? e.message : e) };
...
async function notifyAdminOrder(env, order, extraLines, dedupeKey) {
  ...
  let r = await notifyTelegram(env, html, dedupeKey, {
    parse_mode: "HTML",
    reply_markup: markup,
  });
  if (!r.ok) {
    const plain =
      "PAROKH GOLD · " + (order.kind === "demo" ? "DEMO" : "ORDER") + "\n" +
      "ID: " + (order.id || "") + "\n" +
      "Name: " + (order.name || "") + "\n" +
      "Email: " + (order.email || "") + "\n" +
      "Plan: " + (order.plan || "") + "\n" +
      (order.txid ? ("TXID: " + order.txid + "\n") : "") +
      (order.account ? ("MT5: " + order.account + " / " + (order.broker || "") + " / " + (order.server || "") + "\n") : "") +
      "Reply by email.";
    r = await notifyTelegram(env, plain, dedupeKey, {});
  }
  return r;
}
...
async function loadDemoZipFromTelegram(env) {
  const token = env.TELEGRAM_BOT_TOKEN;
  const fileId = String(env.DEMO_TELEGRAM_FILE_ID || "").trim();
  if (!token || !fileId) return { ok: false, reason: "telegram_file_id_missing" };
  try {
    ...
    let meta = null;
    try { meta = JSON.parse(metaT); } catch (_) {}
    if (!metaR.ok || !meta || !meta.ok || !meta.result || !meta.result.file_path) {
      return {
        ok: false,
        reason: "telegram_getFile_failed",
        detail: String(metaT || "").slice(0, 220),
      };
    }
    const filePath = meta.result.file_path;
    const fileName =
      String(env.DEMO_ZIP_FILENAME || "").trim() ||
      (filePath.split("/").pop() || "ParokhGold-Demo.zip");
    const dl = await fetch(
      "https://api.telegram.org/file/bot" + token + "/" + filePath
    );
    if (!dl.ok) return { ok: false, reason: "telegram_download_http_" + dl.status };
    const buf = await dl.arrayBuffer();
    if (!buf || buf.byteLength < 100) return { ok: false, reason: "telegram_file_empty" };
    // Telegram bot file limit for download path is fine for ~400KB demo zip
    return {
      ok: true,
      filename: fileName.endsWith(".zip") ? fileName : "ParokhGold-Demo.zip",
      content: bytesToBase64(buf),
      bytes: buf.byteLength,
      source: "telegram",
    };
  } catch (e) {
    return { ok: false, reason: "telegram_file_error", detail: String(e && e.message ? e.message : e) };
  }
}

async function loadDemoZip(env) {
  // Priority T: Telegram file_id (no public site hosting)
  const fromTg = await loadDemoZipFromTelegram(env);
  if (fromTg.ok) return fromTg;

  // Optional fallback URL (private link) — not required for path T
  const url = String(env.DEMO_ZIP_URL || "").trim();
  if (url) {
    const r = await fetch(url);
    if (!r.ok) {
      return {
        ok: false,
        reason: fromTg.reason || "demo_url_http_" + r.status,
        detail: fromTg.detail || "",
      };
    }
    const buf = await r.arrayBuffer();
    if (!buf || buf.byteLength < 100) return { ok: false, reason: "demo_url_empty" };
    return {
      ok: true,
      filename: String(env.DEMO_ZIP_FILENAME || "ParokhGold-Demo.zip"),
      content: bytesToBase64(buf),
      bytes: buf.byteLength,
      source: "url",
    };
  ...
  return {
    ok: false,
    reason: fromTg.reason || "demo_zip_not_configured",
    detail: fromTg.detail || "Set DEMO_TELEGRAM_FILE_ID (path T)",
  };
}

async function sendResendEmail(env, { to, subject, html, text, attachments }) {
  ...
  try {
    ...
    if (!r.ok) return { ok: false, reason: "resend_http", detail: body.slice(0, 300) };
    let j = null;
...
function emailTemplate(kind, order) {
  ...
  if (kind === "recv") {
    if (isDemo) {
      return {
        subject: "PAROKH GOLD · درخواست دمو " + id + " دریافت شد",
        text:
          "سلام " + name + "،\n\n" +
          "درخواست دمو شما با کد " + id + " دریافت شد.\n" +
          "فایل دمو به‌زودی از همین ایمیل برایتان ارسال می‌شود.\n\n" +
          "Hello " + name + ",\n\n" +
          "Your demo request " + id + " was received.\n" +
          "The demo package will be sent from this email shortly.\n\n" +
          "— PAROKH GOLD Support\nsupport@parokh.ir\n",
      };
    }
    return {
      subject: "PAROKH GOLD · سفارش " + id + " دریافت شد",
      text:
        "سلام " + name + "،\n\n" +
        "سفارش شما با کد " + id + " دریافت شد و در حال پیگیری است.\n" +
        "ادامه هماهنگی از همین ایمیل انجام می‌شود.\n\n" +
        "Hello " + name + ",\n\n" +
        "Your order " + id + " was received and is being processed.\n" +
        "We will continue from this email.\n\n" +
        "— PAROKH GOLD Support\nsupport@parokh.ir\n",
    };
  }
  ...
  if (kind === "demo") {
    return {
      subject: "PAROKH GOLD · فایل دمو · " + id,
      text:
        "سلام " + name + "،\n\n" +
        "بسته دمو PAROKH GOLD EA پیوست این ایمیل است (کد: " + id + ").\n" +
        "محتوا: فایل اکسپرت دمو + راهنمای پارسی و انگلیسی.\n" +
        "لطفاً طبق راهنما روی MetaTrader 5 نصب کنید.\n\n" +
        "Hello " + name + ",\n\n" +
        "Your PAROKH GOLD EA demo package is attached (ref: " + id + ").\n" +
        "Contents: demo Expert + FA/EN guides.\n" +
        "Please install on MetaTrader 5 using the guide.\n\n" +
        "— PAROKH GOLD Support\nsupport@parokh.ir\n",
    };
  }
  ...
  if (kind === "badtx") {
    return {
      subject: "PAROKH GOLD · سفارش " + id + " · TXID نامعتبر",
      text:
        "سلام " + name + "،\n\n" +
        "TXID ارسال‌شده برای سفارش " + id + " قابل تأیید نبود.\n" +
        "لطفاً TXID صحیح شبکه TRON (USDT TRC-20) و مبلغ دقیق پلن را دوباره از صفحه خرید ارسال کنید.\n\n" +
        "Hello " + name + ",\n\n" +
        "The TXID for order " + id + " could not be verified.\n" +
        "Please resubmit a valid TRON USDT (TRC-20) TXID and the exact plan amount from the purchase page.\n\n" +
        "— PAROKH GOLD Support\nsupport@parokh.ir\n",
    };
  }
  ...
  // deliver (purchase)
  return {
    subject: "PAROKH GOLD · سفارش " + id + " · مرحله تحویل",
    text:
      "سلام " + name + "،\n\n" +
      "پرداخت/سفارش " + id + " تأیید شد.\n" +
      "فایل اختصاصی EX5 برای حساب MT5 شما آماده و از همین ایمیل ارسال/هماهنگ می‌شود.\n\n" +
      "Hello " + name + ",\n\n" +
      "Order " + id + " is confirmed.\n" +
      "Your dedicated EX5 for the licensed MT5 account will be delivered from this email.\n\n" +
      "— PAROKH GOLD Support\nsupport@parokh.ir\n",
  };
}
...
async function saveOrderPatch(env, orderId, patch) {
  const orders = (await kvGetJson(env, "orders", [])) || [];
  const idx = orders.findIndex((o) => o.id === orderId);
...
async function handleTelegramWebhook(request, env) {
  ...
  const cbId = cq.id;
  const m = data.match(/^em:(recv|demo|badtx|deliver):(.+)$/);
  if (!m) {
  ...
  // Guard: action vs order type
  if (kind === "demo" && order.kind !== "demo") {
    await answerCallback(env, cbId, "Only for demo orders");
    return json({ ok: false, error: "not_demo" }, 200, request);
  }
  if ((kind === "badtx" || kind === "deliver") && order.kind === "demo") {
    await answerCallback(env, cbId, "Only for purchase orders");
    return json({ ok: false, error: "not_purchase" }, 200, request);
  }
  ...
  const mail = {
    ...
    subject: tpl.subject,
    text: tpl.text,
    html: "<pre style=\"font-family:Vazirmatn,Tahoma,sans-serif;white-space:pre-wrap;direction:rtl;text-align:right\">" +
      escHtml(tpl.text) + "</pre>",
  };

  if (kind === "demo") {
    const zip = await loadDemoZip(env);
    if (!zip.ok) {
      await answerCallback(env, cbId, "Demo ZIP missing");
      try {
        await notifyTelegram(env,
          "⚠️ Demo ZIP not found.\nSet DEMO_TELEGRAM_FILE_ID (send zip to bot once, copy file_id)\n" +
          (zip.reason || ""),
          null);
      } catch (_) {}
      return json({ ok: false, error: zip.reason || "no_zip" }, 200, request);
    }
    mail.attachments = [{ filename: zip.filename, content: zip.content }];
  }

  const sent = await sendResendEmail(env, mail);

  if (!sent.ok) {
    await answerCallback(env, cbId, "Email failed");
    try {
      await notifyTelegram(env,
        "⚠️ Email FAILED (" + kind + ") " + orderId + " → " + order.email + "\n" +
  ...
  // Patch order status
  const patch = { lastAdminEmail: kind, lastAdminEmailAt: new Date().toISOString() };
  if (kind === "demo") {
    patch.demoSentAt = new Date().toISOString();
    patch.status = "demo_sent";
  } else if (kind === "badtx") {
    patch.paymentStatus = "TXID_REJECTED";
    patch.status = "TXID_REJECTED";
  } else if (kind === "deliver") {
    patch.status = "DELIVERY_NOTIFIED";
  } else if (kind === "recv") {
    patch.status = order.kind === "demo" ? "demo_acknowledged" : "order_acknowledged";
  }
  try { await saveOrderPatch(env, orderId, patch); } catch (_) {}

  await answerCallback(env, cbId, kind === "demo" ? "Demo emailed" : "Email sent");
  try {
    await notifyTelegram(env,
      "✅ " + kind + " → " + order.email + "\nOrder: " + orderId +
      (kind === "demo" ? "\n(attachment: demo zip)" : ""),
