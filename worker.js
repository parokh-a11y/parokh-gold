/**
 * PAROKH GOLD EA — Worker v2.0.8 — website → license_requests
...
function adminEmailKeyboard(order) {
  // Telegram url buttons must be http/https — no mailto (it rejects whole message)
...
async function notifyTelegram(env, text, dedupeKey, options) {
  ...
  try {
    ...
    // Retry without buttons if markup rejected
...
async function notifyAdminOrder(env, order, extraLines, dedupeKey) {
  ...
  // Absolute fallback: short plain text so demo/buy never silent
  if (!r.ok) {
    const plain =
